...
const UserOrders: React.FC<UserOrdersProps> = ({ username }) => {
  ...
  const fetchOrders = async () => {
    try {
      const res = await fetch(`https://deploy-nodejs-vqqq.onrender.com/api/usersProfile/by-username-ordername/${username}`);
  ...
  const handleUpdateOrder = async (orderId: string) => {
    try {
      const res = await fetch(`https://deploy-nodejs-vqqq.onrender.com/api/usersProfile/update-status/${orderId}`, {
        method: 'PUT',
      });
